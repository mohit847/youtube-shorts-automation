...
_font_path = Path(__file__).parent.parent / "assets" / "fonts" / "Roboto-Black.ttf"
...
def create_visualizer_video(audio_path, output_path=None, metadata=None,
                             analysis=None, background_images=None):
    """
...
def _add_watermark_filters(input_label, output_label):
    ...
    filter_str = (
        f"[{input_label}]"
        f"drawbox=x=60:y=100:w=8:h=60:color=red@0.9:t=fill,"
        f"drawtext="
...
def _build_lyric_caption_filters(timed_lyrics, video_duration, input_label, output_label):
    """
...
def _create_image_slideshow_video(audio_path, output_path, images,
    ...
    caption_filters = _build_lyric_caption_filters(
        timed_lyrics, video_duration, "with_watermark", "with_captions"
    )
